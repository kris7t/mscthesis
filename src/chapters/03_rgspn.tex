...
If a symbol \(v\) of variable type is mentioned in an expression, we simply substitute it with its \rel{value}. However, if \(v\) actually refers to a parameter symbol---or is actually parameter symbol---it is instead interpreted to refer to the corresponding element of the parameter vector \(\vec{\uptheta}\). Formally,
\begin{equation}
